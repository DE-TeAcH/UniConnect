...
export function storePin(email, pin, purpose: 'register' | 'reset'): void {
...
export function verifyPin(email, pin, purpose: 'register' | 'reset') {
...
export function hasPendingPin(email) {
    const entry = pinStore.get(email.toLowerCase());
    if (!entry) return false;
    if (Date.now() > entry.expiresAt) {
        pinStore.delete(email.toLowerCase());
